    ...
    if inFeatureSet or inFeatureSet != "#":
        try:
            ...
            if countRows != tablerowcount:
                ...
                causeTablefieldsName = [i.name for i in  arcpy.ListFields(causeTable)]
                Desc_idx = causeTablefieldsName.index('DESCRIPTION')
                Desc_list= []
                with arcpy.da.UpdateCursor(causeTable,causeTablefieldsName) as causecursor:
                    for row in causecursor:
                        Desc_list.append(row[Desc_idx])
                del causecursor

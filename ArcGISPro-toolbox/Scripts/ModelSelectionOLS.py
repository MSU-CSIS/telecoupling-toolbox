...
def runExploratoryRegression():
    ...
    outputReportFile = ARCPY.GetParameterAsText(4)
    #### Search Criterion ####
    maxIndVars = int(ARCPY.GetParameterAsText(5))
    minIndVars = int(ARCPY.GetParameterAsText(6))
    minR2 = float(ARCPY.GetParameterAsText(7))
    maxCoef = float(ARCPY.GetParameterAsText(8))
    maxVIF = float(ARCPY.GetParameterAsText(9))
    minJB = float(ARCPY.GetParameterAsText(10))
    minMI = float(ARCPY.GetParameterAsText(11))
    print((maxIndVars,minMI))
...
class ResultHandler(object):
    ...
    def evaluateResult(self, olsResult, residuals, keep = False):
        ...
        if olsResult.r2 is not None and self.minR2 is not None:
            r2Bool = olsResult.r2 >= self.minR2
        else:
            r2Bool = True
        if not self.allJBPass and olsResult.jb is not None and self.minJB is not None :
...
class OLSResult(object):
    ...
    def evaluateVIF(self, maxVIF = 5.0):
        ...
        if self.k >= 2:
            ...
            if self.vifVals is not None and maxVIF is not None:
    ...
    def evaluatePVals(self, maxCoef = .01):
        ...
        if self.pVals is not None and maxCoef is not None:
...
class ExploratoryRegression(object):
    ...
    def __init__(self, ssdo, dependentVar, independentVars, weightsFile,
                 outputReportFile, maxIndVars = 5, minIndVars = 1, minR2 = .5,
        ...
        for resKey, resHandler in UTILS.iteritems(self.resultDict):
            for olsKey, olsRes in UTILS.iteritems(resHandler.olsResults):
                ...
                if miValue is not None:
                    ...
                    self.sumMoranRuns += 1
                    if miValue is not None and self.minMI is not None and miValue > self.minMI:
                        self.sumMoranPass += 1
    ...
    def endSummary(self):
        ...
        if self.minR2 is None:
            self.minR2 = NUM.nan
        if self.maxCoef is None:
            self.maxCoef = NUM.nan
        if self.maxVIF is None:
            self.maxVIF = NUM.nan
        if self.minJB is None:
            self.minJB = NUM.nan
        if self.minMI is None:
            self.minMI = NUM.nan

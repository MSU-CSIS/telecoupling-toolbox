...
def PopCountDensity(inFeature, featureField, inCensus, censusField, outFC, index):
    ...
    try:
        ...
        shapeFieldName = arcpy.Describe(inCensus).shapeFieldName
        calcExpression = "!{0}.AREA@SQUAREKILOMETERS!".format(shapeFieldName)
        arcpy.AddField_management(inCensus, "popArea", "DOUBLE")
        arcpy.CalculateField_management(inCensus, "popArea", calcExpression, "PYTHON")

        #Set up a calculation expression for the density calculation
        calcExpression = "!" + censusField + "!/!" + 'popArea' + "!"
        #Calculate the population density
        arcpy.AddField_management(inCensus, "popDens" + index, "DOUBLE")
        arcpy.CalculateField_management(inCensus, "popDens" + index, calcExpression, "PYTHON")
        ...
        shapeFieldName = arcpy.Describe(intersectOutput).shapeFieldName
        calcExpression = "!{0}.AREA@SQUAREKILOMETERS!".format(shapeFieldName)
        arcpy.AddField_management(intersectOutput, "intArea", "DOUBLE")
        arcpy.CalculateField_management(intersectOutput, "intArea", calcExpression, "PYTHON")
        ...
        for field in fieldList:
            if "sum_pop" in field.name.lower():
                ...
                arcpy.CalculateField_management(os.path.join(arcpy.env.scratchGDB, tempName), pc_name, "!" + popVar + "!", "PYTHON")
        ...
        for field in fieldList:
            if "sum_int" in field.name.lower():
                ...
                arcpy.CalculateField_management(os.path.join(arcpy.env.scratchGDB, tempName), area_name, "!" + areaVar + "!", "PYTHON")
                arcpy.DeleteField_management(os.path.join(arcpy.env.scratchGDB, tempName), areaVar)

        #Calculate the population density
        arcpy.AddMessage("Calculating population density...")
        calcExpression = "!" + pc_name + "!/!" + area_name + "!"
        popDens = "popDens" + index
        arcpy.AddField_management(os.path.join(arcpy.env.scratchGDB, tempName), popDens, "DOUBLE")
        arcpy.CalculateField_management(os.path.join(arcpy.env.scratchGDB, tempName), popDens, calcExpression, "PYTHON")        
        ...
        #The first step in the else statement is to find a field that will be used to join the two time points.
            ...
            if OID_1 != "" and OID_2 != "":
                ...
                calcExpression = "((!popCount_2! - !popCount_1!) / !popCount_1!) * 100"
                arcpy.AddField_management(os.path.join(arcpy.env.scratchGDB, tempName_1), "PopGrowth", "DOUBLE")
                arcpy.CalculateField_management(os.path.join(arcpy.env.scratchGDB, tempName_1), "PopGrowth", calcExpression, "PYTHON")
